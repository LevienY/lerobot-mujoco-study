...
def _unwrap_policy(policy):
    ...
    PEFT (PeftModelForFeatureExtraction.forward) 会把 batch dict
    拆成 **kwargs 传给底层模型，但 PI0Policy.forward() 只接受
    一个 batch dict 参数（不接受 input_ids 等 keyword argument），
    因此必须绕过 PEFT 的 forward，直接调用底层模型的 forward。

    LoRA 的 A/B 矩阵已经注入到底层模型的 Linear 层里了，
    绕过 PEFT forward 不会丢失 LoRA 的计算。
    """
    inner = policy
    # 穿透 DDP wrapper
...
def update_policy(
    ...
    actual_model = _unwrap_policy(policy)
    ...
    if has_method(actual_model, "update"):
...
def train(cfg: TrainPipelineConfig):
    ...
    if lora_rank > 0 and HAS_PEFT:
        ...
        if not target_modules:
            ...
            target_modules = "all-linear"  # peft 特殊值：所有 Linear 层
    ...
    for _ in range(step, cfg.steps):
        ...
        if is_log_step and local_rank == 0:
            ...
            if wandb_logger:
                ...
                if output_dict:
                    # 只保留 scalar（0-d tensor / float / int），过滤掉 1-d+ tensor
                    for k, v in output_dict.items():
                        if isinstance(v, torch.Tensor) and v.ndim == 0:
                            wandb_log_dict[k] = v.item()
        ...
        if cfg.save_checkpoint and is_saving_step and local_rank == 0:
            ...
            if lora_rank > 0 and HAS_PEFT and isinstance(model_to_save, peft.PeftModel):
                ...
                elif hasattr(base_config, 'to_dict'):
